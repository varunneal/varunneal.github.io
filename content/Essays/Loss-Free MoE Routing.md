...
type: technical
---
**Overview:** The two things a good MoE router needs are load balancing and geometric stability. I propose two optimizers for routers that target these properties: the first constrains router weights to the Stiefel manifold; the second extends this into a fully loss-free update that achieves balance and orthogonality without any gradient from the language modeling objective.
...
**Algorithm 1:** Simplified shampoo update. Barring details on accumulation and approximation, the above is identical to Muon.[^oldnorm][^anil] The `left_precond` factor blends gradient signals across rows. Expert $j$'s update is contaminated by expert $i$'s gradient, even though the two never share tokens.
...
def manifold_muon_update(
    ...
    dual_lr: float = 0.01, dual_steps: int = 20,
) -> Tensor:
    G = G / (G.norm() + 1e-8)
...
def loss_free_router_step(W, x, logits, eta):
    ...
    G = scale.unsqueeze(1) * (w.T @ x)
    # 2. manifold muon update
    return manifold_muon_update(W, G, eta)
```
...
def manifold_muon_step(
    ...
    rho: float = 1.0, admm_steps: int = 20, preserve_mag: bool = False,
) -> Tensor:
...
```
**Algorithm x: Manifold Muon update with a few tricks**
...
- `streaming_msign` uses Jianlin Su's Streaming SVD[^su] algorithm. The reason for this is to encourage the final `msign` to have the same order of singular vectors from step-to-step. Newton-Schulz (and similar algorithms) may noisily shuffle rows in early training. 
...
For the Loss Free optimizer, we use the same balancing loss as GShard, but compute its gradient explicitly rather than through autograd.

Let $R \in \mathbb{R}^{E \times D}$ be the router weight, with rows $R_j$ corresponding to each expert's routing vector. Given token activations $x_1, \dots, x_T \in \mathbb{R}^D$, the routing score for token $i$ to expert $j$ is $\rho_{i,j} := \sigma(x_i \cdot R_j)$, where $\sigma$ is the sigmoid.[^sigmoid] The mean routing score per expert is $F_j := \frac{1}{T} \sum_{i=1}^T \rho_{i,j}$, and $F = [F_1, \dots, F_E] \in \mathbb{R}^E$ is the vector of all expert loads.

[^sigmoid]: The routing score could just be the raw dot product $x_i \cdot R_j$, but sigmoid keeps scores in $(0, 1)$ and gives a clean derivative $\sigma'(z) = \sigma(z)(1-\sigma(z))$. Any monotonic function of dot-product similarity works here though the resulting gradient will differ slightly.
...
We want $\frac{\partial \mathcal{L}}{\partial R_j}$. By the chain rule:

$$
\frac{\partial \mathcal{L}}{\partial R_j} = \frac{\partial \mathcal{L}}{\partial F_j} \cdot \frac{\partial F_j}{\partial R_j}
$$
...
For the second term, since $F_j = \frac{1}{T}\sum_i \sigma(x_i \cdot R_j)$ and $\sigma' = \sigma(1-\sigma)$:

$$
\frac{\partial F_j}{\partial R_j} = \frac{1}{T}\sum_{i=1}^T \rho_{i,j}(1 - \rho_{i,j}), x_i
$$

Combining:

$$\boxed{\frac{\partial \mathcal{L}}{\partial R_j} = 2(F_j - \bar{F}) \cdot \frac{1}{T}\sum_{i=1}^T \rho_{i,j}(1-\rho_{i,j})\, x_i \;\in \mathbb{R}^{D}}$$

The full gradient $\nabla_R \mathcal{L} \in \mathbb{R}^{E \times D}$ stacks these rows. In matrix form, let $W = \rho \odot (1 - \rho) \in \mathbb{R}^{T \times E}$:

$$
\nabla_R \mathcal{L} = \frac{2}{T}\, \text{diag}(F - \bar{F})\; W^\top X
$$
...
3. Capacity-maximization (see appendix note on capacity)
4. Sequence-level load balancing
5. Domain specialization loss
...
[^tilde]: Keigwin, Pai, Chen (Tilde Research) 2025 Gram-Space Manifold Muon
...
All experiments use a 12-layer, 768-dimensional NanoGPT-style transformer. Every layer's MLP is replaced by a 64-expert SwiGLU MoE with top-2 routing, giving ~1.5B total parameters of which ~147M are active per token. Routing uses sigmoid gating renormalized to sum to one.
