...
[^geometry]: As mentioned in a previous footnote, orthogonalization corrects the geometry of the update to match the geometry of the parameter space. By "well-rounded", I mean that the update is normalized with respect to the same gradient as the parameter space. Intuitively, I think of this as "normalization w.r.t. directionality", and imagine an ellipsoid update becoming spherical. We can alternatively view orthogonalization as constraining the parameter space. If each update is orthogonal (all singular values are $1$),  the weights themselves will be well-conditioned (singular values are somewhat near $1$). Even though orthogonalization is approximate, Muon is effectively performing gradient descent down a constrained submanifold of the full parameter space. Along these lines, Jeremy Bernstein has proposed [Manifold Muon](https://thinkingmachines.ai/blog/modular-manifolds/), which tweaks Muon such that the weights remain nearly-orthogonal. 
