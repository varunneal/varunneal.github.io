...
## (1) Adaptive Muon
There have been several recent papers highlighting the benefits of an adaptive estimation term inside of Muon [^normuon] [^adamuon] [^adago] [^frans]. One of these methods (Normuon) incorporated into the speedrun on October 24th. 
...
[^adamuon]:[Si et al 2025] *AdaMuon*

Stochastic Gradient Descent with Momentum (SGDM) performs steps by computing the exponential moving average (EMA) of gradients. Adam (ADaptive Moment Estimation) normalizes the EMA by tracking the exponential moving average of the norm of each parameter. 
...
[^kustner]: [Kustner et al 24](https://arxiv.org/abs/2402.19449) *Heavy-Tailed Class Imbalance and Why Adam Outperforms Gradient Descent on Language Models*
[^cohen]: [Cohen et al 24](https://arxiv.org/abs/2410.24206) *Understanding Optimization in Deep Learning with Central Flows*, with a shorter accompanying blogpost [here](https://centralflows.github.io/part3/). 
...
In various works, the update Muon has been argued to solve (2)—e.g. it is "curvature aware" [^kovalev][^anonymous26][^su25]. Intuitively, this is a feature of orthogonalizing the update.. After orthogonalization, Muon's update is "well-rounded" in the parameter space, avoiding directions of steep change such as sharp crevasses [^spectral]. 
...
Empirically, we find that since each step we is perfectly-conditioned (all spectral values are $1$), the parameter weights tend to remain similarly well-conditioned (the spectral values are near each other)[^Boreiko]. Muon is effectively performing gradient descent down a well-behaved submanifold of the full parameter space. Relatedly, Jeremy Bernstein has proposed *Modular Muon*, which tweaks Muon such that the weights themselves remain semi-orthogonal through gradient descent [^bernstein25].
...
A power-law relationship ($\eta \propto B^p$) is often assumed to hold for Muon as well[^sato][^ryu], though in practice I discovered this is only true at low batch sizes.
...
**Figure 2: Sweeping Adaptive Muon at a learning rate x log(batch size) grid at three different token budgets. The lowest loss per batch size is selected in red. See [[#(3) Implementation Notes]] for more details.** 
...
Below the asymptote/saturated batch size, we can approximate the value $p$ in $\eta \propto B^p$ via the slope of the red boxes. At this granularity, $p \approx 0.6-1.0$ seems approximately correct. Convergence to the optimal learning rate appears to be very fast for this variant of Muon.
...
Last month, a paper from Shulgin et al demonstrated that a more precise orthogonalization genuinely does improve Muon convergence, especially when accompanied with appropriate learning rate tuning: 
...
It follows that using Polar Express over Newton Schulz represents a strict improvement in convergence.
...
Inside Modded NanoGPT, weight decay on Muon doing more harm than good. In October, a variant of decoupled weight decay known as *Cautious Weight Decay* was discovered [^chen25], which only decays parameters that increase in magnitude in the update step:
...
The implementation of Muon has been optimized in order to distribute the implementation over 8 devices. While some of these engineering tricks are interesting (e.g. batched orthogonalization), they do not represent algorithmic differenes from the original form of Muon. For additional information I direct you to Larry Dial's blog post [^larry2025a] and the Modded NanoGPT repo [^moddednanogpt]. 


## (6) Implementation Notes 
I hope this post is broadly useful for to pretraining with Muon. I want to highlight some important considerations for the Modded NanoGPT recipe: 
...
- Adam is stepped over twice the number of gradient accumulation steps than Muon. Effectively, it has a 2x batch size than Muon. In the experiment in [[#(2) Batch size scheduling|Section 2]], I've scaled the Adam learning rate according to the square-root law. 
%% 
...
[^snr]: I'm being somewhat imprecise with how we define the signal to noise ratio. McCandlish et al [^mccandlish] defines a critical batch size $B_{\text{noise}} =  \text{Cov}(G) / \mathbb{E}(G)^2$ for their analysis of SGD, which is the inverse SNR. 
